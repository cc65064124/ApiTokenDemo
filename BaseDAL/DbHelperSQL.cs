    ...
    public abstract class DbHelperSQL
    {
        ...
        public static int ExecuteSqlTran(string[] SQLStringArr)
        {
            ...
            using (SqlConnection conn = new SqlConnection(connectionString))
            {
                ...
                {
                    int count = 0;
                    for (int n = 0; n < SQLStringArr.Length; n++)
                    {
                        string strsql = SQLStringArr[n];
                        if (strsql.Trim().Length > 1)
                        {
                            strsql = FiltrationDangerChar(strsql);
                            cmd.CommandText = strsql;
        ...
        /// <param name="parameters">存储过程参数</param>
        /// <param name="tableName">DataSet结果中的表名</param>
        /// <param name="out rCount">记录总数</param>
        /// <param name="out pCount">分页总数</param>
        /// <returns>DataSet</returns>
        public static DataSet RunProcedure(string storedProcName, IDataParameter[] parameters, string tableName, out int rCount, out int pCount)
        {
            ...
            using (SqlConnection connection = new SqlConnection(connectionString))
            {
                ...
                connection.Open();
                SqlDataAdapter sqlDA = new SqlDataAdapter();
                SqlCommand sqlCmd = BuildQueryCommand(connection, storedProcName, parameters);
                sqlDA.SelectCommand = sqlCmd;
                sqlDA.Fill(dataSet, tableName);
                //从输出参数中读取[总记录数]
                rCount = (int)sqlCmd.Parameters["@rCount"].Value;
                //从输出参数中读取[总页数]
        ...
        /// <param name="parameters">存储过程参数</param>
        /// <returns></returns>
        public static int RunProcedureGetAffectedCount(string storedProcName, IDataParameter[] parameters)
        {
            using (SqlConnection connection = new SqlConnection(connectionString))
            {
                connection.Open();
        ...
        /// </summary>
        /// <param name="connection">conn对象</param>
        /// <param name="storedProcName">存储过程名</param>
        /// <param name="parameters">存储过程参数</param>
        ...
        public static string GetValueByTableAndFiled(string tableName, string filedName, string Where)
        {
            ...
            strSql.Append(" select " + filedName + " from " + tableName + "");
        ...
        /// <param name="pSize">一页显示多少条</param>
        ...
        public static DataSet GetRecordByPage(string pSql, int pNum, int pSize, string sort, string rowNumName, out int rCount, out int pCount)
        {
            ...
            SqlParameter[] parameters = {
					new SqlParameter("@pSql", SqlDbType.NVarChar, 4000),
					new SqlParameter("@pNum", SqlDbType.Int),
					new SqlParameter("@pSize", SqlDbType.Int),
					new SqlParameter("@sort", SqlDbType.NVarChar),
					new SqlParameter("@rowNumName", SqlDbType.NVarChar,100),
